...
def export_to_executorch_with_cuda_windows(
    ...
    **kwargs,
    ...
    def _generate_compile_spec(
        key: str,
        value: str,
    ) -> List[CompileSpec]:
        return CompileSpec(key, value.encode("utf-8"))
    ...
    def _lower_to_executorch(
        ...
        partitioners = {
            key: [
                CudaPartitioner(
                    [
                        CudaBackend.generate_method_name_compile_spec(key),
                        _generate_compile_spec("platform", "windows"),
                        _generate_compile_spec("shim_library_path", "/tmp/shim"),
                    ]
                )
            ]
